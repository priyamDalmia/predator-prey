...
class AACAgent(BaseAgent):
    def __init__(self,
        ...
        def get_action(self, observation):
            ...
            log_probs = action_dist.log_probs(action)
            return action.item(), log_probs

        def train_step(self):
            states, action, rewards, done = \
                    self.sample_transition()
                if len(states) = 0
                    return 0
            
            states = torch.as_tensor(states, device=self.deivce)
            ...
            log_probs = probs_dist.log_probs(actions)
        ...
        def sample_transition(self):
            ...
            self.initialize_memory(self.input_dims)
            return states, action, rewards, dones
