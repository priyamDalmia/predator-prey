...
class gather_v1(ParallelEnv):
    ...
    def step(
        ...
        for agent_id in list(self._agents):
            ...
            if agent_id in shots_hit:
                self._game_history.loc[self._game_step, f"{agent_id}_hit"] = 1
    ...
    def predators_in_line_of_fire(self, position, direction):
        ...
        tiles_affected = []
        if direction == 1: # UP 
            tiles_affected = [(position[0] - i, position[1]) for i in range(1, self.pred_vision+1)]
        elif direction == 2: # DOWN
            tiles_affected = [(position[0] + i, position[1]) for i in range(1, self.pred_vision+1)]
        elif direction == 3: # RIGHT
            tiles_affected = [(position[0], position[1] + i) for i in range(1, self.pred_vision+1)]
    ...
    def get_observation(self, agent_id):
        ...
        for k, v in self._predators.items():
            if not v.is_alive:
                continue 
            if v.is_stunned:
                state[v.position[0], v.position[1], self.PREDATOR_CHANNEL] = -1 * (v._stun_count)
        observation = state[
    ...
    for i in range(100):
        ...
        while env.agents:
            ...
            actions = {
                ...
                "predator_1": agressive_agent.get_action(obs["predator_1"]),
            }
